...
리스트는 "변경 가능(mutable)" 하며, 이는 "제자리(in place)"에서 수정할 수 있음을 의미합니다.

리스트를 수정하는 한 가지 방법은 인덱스 또는 슬라이스를 사용하는 방법입니다.
...
파이썬은 리스트를 사용하는데 매우 유용한 몇 가지 함수들을 제공합니다.
...
<!-- tab MAX @code -->
{% note primary no-icon %}
  이전에 `min` 및 `max` 를 사용하여 최소 또는 최대 인수를 구해봤습니다. 우리는 하나의 리스트를 매개 변수로 전달할 수도 있습니다.
...
하지만 리스트에는 항상 자주 사용되는 몇 가지 메소드들이 있습니다.

## 리스트 메소드들(List methods)

`list.append` 를 사용하여 리스트의 마지막에 item을 추가할 수 있습니다:
...
`list.pop` 은 리스트의 마지막 element를 제거하고 반환합니다:
